...
- [X] **9 月 27 日活动出发时间调整** (2026-09-24)
  - [X] 中英德三语活动页仅将出发时间由 09:00 改为 10:00（慕尼黑当地时间），其他活动内容与线上旧版逐字核对一致。
  - [X] 报名 API 无该活动数据库记录，无需同步已有报名；前端检查和构建通过。
  - [X] 主分支提交 `de3138a` 已部署，三语公开详情页均显示 10:00 并返回 HTTP 200。

- [X] **9 月 27 日活动封面更新** (2026-09-24)
  - [X] 将用户裁好的 `5384.heic` 无裁切转为 3024×2268 JPEG，按活动素材规范保存为 `cover.jpg`，更新三语正文图片说明。
  - [X] 前端检查 0 errors / 0 warnings，生产构建通过；主分支提交 `b987212` 的 Vercel 部署成功。
  - [X] 三语活动页面返回 HTTP 200，线上封面返回 HTTP 200，线上图片 SHA-256 与本地文件一致。

- [X] **9 月 27 日蛋糕冰淇淋周日休闲骑** (2026-09-24)
  - [X] 发布中英德三语活动页：09:00 从 Waldfriedhof Eingang Süd 出发，73.1 km / 510 m，标明三个歇脚点、巡航上限、天气穿着提醒和 ACC 小礼物。
  - [X] 前端检查 0 errors / 0 warnings，生产构建通过；主分支提交 `d6f1b92` 的 Vercel 前后端部署成功。
  - [X] 三语活动详情页及列表均返回 HTTP 200，路线和出发信息齐全，封面图片返回 HTTP 200。

- [X] **湖畔轻松骑配图** (2026-09-16)
  - [X] 将之前同路线活动的图片复制到本次活动目录，三语页面设为封面并加入正文插图，注明为往期活动配图。
  - [X] 前端检查 0 errors / 0 warnings，生产构建通过。

- [X] **9 月 19 日湖畔轻松骑** (2026-09-16)
  - [X] 按活动 Word 文档准备中英德三语页面：09:45 集合，80 km / 570 m，更新 Komoot 路线及两个可选返程节点。
  - [X] 前端检查 0 errors / 0 warnings，生产构建通过。
  - [X] 已发布：`e399d46`；Vercel 前后端部署成功，三语公开页面均返回 HTTP 200，路线、里程及报名组件核验通过。
...
- [X] **About Us 原版山谷标识** (2026-09-07)
  ...
  - [X] Astro 检查 0 errors / 0 warnings、前端 70 项单元测试、生产构建及三语产物引用/图片哈希核验通过；桌面浏览器 DOM 无横向溢出。
  - [ ] 浏览器截图与视口/主题交互工具异常，手机和深色主题视觉效果待人工复核。

- [X] **报名邮件 9:30 错显 11:30 的时区修复** (2026-09-05)
  - [X] 复现 UTC 服务器将无时区活动时间当 UTC 的根因；活动日期/截止时间字符串统一按慕尼黑解析，保留带时区的真实瞬间，拒绝 DST 歧义及非法日期。
  - [X] 9 月 6 日 Epic Ride 三语源文件明确为 `09:30+02:00`，API 提交 `07:30Z`；Python Markdown 同步支持显式时区 ISO 字符串。
  - [X] 对齐“所有活动默认慕尼黑时间”：报名、创建及后台同步输入、活动日期显示、当天边界、周期活动截止覆盖值和默认前日 22:00 截止均采用 Europe/Berlin，存储仍为 UTC。
  - [X] 后端 242 项、前端 70 项单元测试、两种服务器时区检查及桌面/手机 2 项真实报名表回归通过；纽约时区设备提交的出发/截止时间正确，三语邮件均为 `09:30 CEST`；Astro 检查和 UTC 构建通过。
  - [X] 已只读确认线上活动 ID 76 仍存为错误的 `09:30Z`。修复尚未发布，未改线上数据、未发通知；条件化纠正 SQL 和发布顺序见 `docs/deployment/registration_timezone.md`。

- [X] **取消报名邮件说明排版** (2026-09-05)
  - [X] 取消说明移至路线/名单之后、取消按钮上方；个人专属链接提示独立置于按钮下，采用 12px 浅灰小字，中英德文及纯文本同步。
  - [X] 更新正式邮件样稿；26 项邮件测试通过，已生成并检查桌面/手机完整截图，注释位置与窄屏溢出检查通过。未部署、未发送邮件。

- [X] **报名者自助取消与邮件入口** (2026-09-05)
  - [X] 报名确认、活动提醒和候补回执增加中英德三语“取消我的报名”按钮与说明，HTML 和纯文本同步；更新真实邮件样稿。
  - [X] 个人邮件链接无需登录即可管理报名，页面二次确认后取消；原有名单链接兼容，支持保留报名、失败重试及刷新后的已取消状态。
  - [X] 只允许出发前且未签到的本人报名取消；校验活动与 token、重复操作幂等、事务失败回滚、人数同步、候补递补及确认邮件，保留 Ride Leader 重算。
  - [X] 后端完整 229 项、前端 59 项单元测试及桌面/手机 12 项浏览器测试通过；Astro 检查和构建通过，新后端模块 lint 通过。
  - [X] 已检查手机报名确认界面及无图邮件渲染。尚未部署、未发送真实邮件；无需数据库迁移，发布与线上验收见 `docs/deployment/rsvp_self_cancellation.md`。

- [X] **邮件联系页脚文案** (2026-09-05)
  - [X] 中文页脚改为“有疑问？写信联系我们 📧”，整句可点击发信；英文和德文同步对应语气，纯文本保留邮箱地址。
  - [X] 已重新生成邮件样稿并检查链接和宽/窄屏渲染。

- [X] **邮件字号层级与 Komoot 入口细化** (2026-09-05)
  - [X] 标题 22px、正文 15px，说明/署名字号与行距统一；补充系统字体回退，书法缩至 180px 宽，减少拥挤与粗重感。
  - [X] 沿用网站路线页的 Komoot 绿 `#6AA127`，仅路线按钮采用浅绿色底、绿色细描边及深绿色文字，其他操作保留 ACC 红；按钮更紧凑并加 5px 圆角，信息卡及邮件外框为 8px 圆角。
  - [X] 更新全部真实模板样稿与旧改期预览；16 项相关后端测试、14 个宽/窄屏邮件渲染及无图/纯文本检查通过，lint 通过。旧邮件客户端可退回直角，真实收件验证仍待完成。

- [X] **ACC 邮件出发信息卡与社区署名** (2026-09-05)
  - [X] 按选定的 B 方案统一订阅回执、报名确认和改期通知；使用报名/订阅姓名，报名中文采用欢迎参加及“不见不散”的语气，订阅/报名同步中英德文。
  - [X] 改期通知去掉项目符号，原因和原时间写入正文，仅突出新日期时间及集合点；署名为 `Let's Ride, Free and Together`，下附 `Across Paths · Mountains · Borders` 和原有“穿越无疆”PNG 书法。
  - [X] 共享纯渲染器输出 HTML 与纯文本，使用表格/内联样式/系统字体及 Outlook 宽度回退；保留路线、名单、可选群二维码和退订入口，对动态内容转义。
  - [X] 真实渲染器生成七个邮件样稿及切换页；14 个宽/窄屏渲染、关闭图片后的关键信息与纯文本预览检查通过，后端完整 209 项测试通过，新模块 lint 通过。
  - [X] 公开书法 PNG 返回 200。未发送真实邮件；Gmail、Outlook、Apple Mail 收件箱及深色模式实测待完成，浏览器预览不代替邮箱兼容验证。

- [X] **Dashboard 日期改期与操作入口梳理** (2026-09-05)
  - [X] 通知区拆为发送骑行提醒、更改日期与时间、取消整个活动三个明确入口，说明各自的数据变更和邮件效果；改期/取消不需要另外发送提醒。
  - [X] 支持选择慕尼黑当地日期和时刻，审核原/新完整时间；保留报名、候补、活动链接及原报名截止时间。旧客户端不传日期时兼容同日改时，无需新增迁移。
  - [X] 跨日、跨年、午夜换算、非法日期和夏令时校验通过；已取消活动禁止普通提醒，提醒发送的跳过/失败计数准确呈现。
  - [X] 后端完整 200 项测试、前端 59 项单元测试、桌面/手机 14 项 Dashboard 浏览器测试通过；Astro 检查 0 error / 0 warning，构建通过，并检查操作卡片、日期表单和审核框截图。
  - [X] 更新部署说明：先部署支持日期的后端，再部署前端。未发送真实邮件，邮件风格建议稿仍独立保留。

- [X] **Garage 详情动效简化** (2026-09-05)
  - [X] 打开单车详情时仅原地轻晃一次，约 0.65 秒后停稳；移除飞出、返回及临时图片层。
  - [X] 保留车库横向自动循环、无暂停按钮、手机上图下文及减少动态效果支持。
  - [X] 桌面/手机浏览器测试 10 项通过；Astro 检查 0 error、0 warning（33 项已有 hints）。

- [X] **Garage 自动循环与单车驶出返回** (2026-09-05)
  - [X] 按屏宽展示完整卡片，以重复轨道无缝自动循环、柔和边缘替代硬裁切；按用户要求移除左右箭头和暂停按钮。
  - [X] 支持鼠标/触摸拖动，悬停及键盘聚焦临时停止；关闭详情后可继续，减少动态效果设置下保留手动浏览。
  - [X] 点开详情后单车先轻晃，向右驶出屏幕，再从左侧返回原位；保留手机上图下文，关闭或调整窗口时清理动画。
  - [X] 10 项桌面/手机浏览器测试通过，包含完整循环、正确飞行方向、拖动防误触、长简介滚动及 reduced motion；前端单元测试 58 项通过，类型检查及构建通过。

- [X] **About 山脊配色与纹理衔接** (2026-09-05)
  - [X] 将原 SVG 约 40% 的绿色线条面积替换为蓝/红色，保留其余笔触。
  - [X] 撤去不协调的细线补段，复用原图带断笔和粗细变化的山脊片段衔接下方空白；修正近景渐变蒙版中的无效颜色写法。
  - [X] 已在桌面及手机浏览器渲染检查，保留原有远近两层山脊布局。

- [X] **Dashboard 出发时间更改与通知** (2026-09-05) — 分支 `phase-11/garage-event-updates`
  - [X] 原取消操作扩展为“原因 → 取消/改时 → 审核并发送”；按慕尼黑时区调整当天出发时刻，保留报名及候补状态。
  - [X] 新增改时英文邮件，明确原时间、新时间、原因及集合点；数据库先提交，页面持续显示发送/跳过/失败数量。
