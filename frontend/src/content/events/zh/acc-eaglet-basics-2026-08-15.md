---
slug: acc-eaglet-basics-2026-08-15
title: 雏鹰计划 3.0 · 第一课｜基础安全
description: 装备检查、道路安全、刹车与变速，20–30 km 理论结合实骑。
location: Am Bavariapark 5, 80339 München-Schwanthalerhöhe
author: ACC Club
date: 2026-08-15 09:30
eventType: training-camp
cover: /images/events/acc-eaglet-basics-2026-08-15/cover.jpg
displaySections:
  - hero
  - upcoming
heroPriority: 1
status: published
ACCOfficialRide: true
distanceKm: 30
routeKomootUrl: https://www.komoot.com/de-de/tour/3190190865?share_token=axmnC31fAwZrtZF0OaxMDJbysi0KMcgA3sTf4TjYHY0zm8Bf5x&ref=wtd&t_s=referral&t_cid=route_share&t_ref_username=2285951965613
registrationLink: ''
---

**雏鹰计划 3.0 第一课。目标：建立正确的骑行习惯。**

> 这是三期系列课程的第一期。**每一期需要单独报名。**
> 第二课｜团骑技术 → [8 月 22 日（周六）](/zh/events/acc-eaglet-groupride-2026-08-22)
> 第三课｜实战提升 → [8 月 29 日（周六）](/zh/events/acc-eaglet-longride-2026-08-29)

第一课解决三件事：出门前该检查什么、在德国的路上该怎么骑、跟一群人一起骑时该守什么规矩。

不需要任何团骑经验——这正是这门课要教的。

***

|  |  |
| --- | --- |
| **时间** | 2026 年 8 月 15 日（**周六**）**9:30 出发** |
| **集合地点** | **Am Bavariapark 5, 80339 München-Schwanthalerhöhe** |
| **距离 / 强度** | **20–30 km**，轻松节奏，全程有领骑带队 |
| **路线** | [Komoot 查看路线](https://www.komoot.com/de-de/tour/3190190865?share_token=axmnC31fAwZrtZF0OaxMDJbysi0KMcgA3sTf4TjYHY0zm8Bf5x&ref=wtd&t_s=referral&t_cid=route_share&t_ref_username=2285951965613) |
| **课程费** | ACC 会员**免费** · 非会员学生 **10 €** · 非会员非学生 **15 €** |
| **注意事项** | 头盔必戴；请携带饮水与基础补给 |

> 如遇恶劣天气，课程可能顺延至周日，届时另行通知。

***

## 课程内容

### 第一部分：骑行前准备

- **人身装备**：头盔、眼镜、手套、锁鞋、骑行服
- **ABC 出发检查**：气压、刹车、传动、快拆 / 桶轴、整车复查
- **随车工具**：备用内胎、撬胎棒、打气筒 / CO₂、多功能工具、链条快扣

### 第二部分：德国骑行交规

- 自行车道路权：什么时候**必须**走自行车道（Zeichen 237 / 240 / 241）
- Fahrradstraße、Fahrrad Zone、30 区
- **Rechts vor Links** 与路口优先权
- 红绿灯规则、左转的两种方式
- 常见德国自行车标志

### 第三部分：团骑基础

**团骑规范**——手势、口令、单列 / 双列、超车、安全距离、如何通过路口。

其中最重要的一条：**严禁叠轮**。这是团骑摔车最主要的单一原因。

### 第四部分：车辆基础

坐高调整、握姿、变速基础、合理踏频、正确刹车。

重点解决：**为什么踩着累？为什么总是踩空？为什么换挡不顺？**

### 第五部分：实操训练

练习起步、停车、换挡、高踏频、紧急刹车、转弯、简单跟车。

***

## 当日安排

1. 在 **Theresienwiese** 绕圈热身，练习熟悉骑行手势
2. 骑往 **Regatta**，途中实地经过 30 区（rechts vor links）、Fahrrad Zone、自行车道、人车共用道与机动车道、红绿灯——理论结合实践
3. **Kaffeepause**
4. 前往 **Fröttmaninger Müllberg** 爬小坡，实践上下坡中的踏频、换挡与刹车
5. 总结与 Q&A，结束后在 Müllberg 自行解散

***

## 课前预习

理论部分我们已经整理成文章，报名后先读一遍，上课时会轻松很多：

- [出发前准备：装备清单与 ABC 快速检查](/zh/knowledge/training/pre-ride-check-abc)
- [德国骑行交规与路权：新手必读](/zh/knowledge/training/german-cycling-traffic-rules)
- [团骑规范：手势、队形与安全距离](/zh/knowledge/training/acc-group-riding-rules)
- [车辆基础设置：座高、握姿与变速调整](/zh/knowledge/training/bike-setup-basics)
- [踏频、换挡与刹车：三项核心骑行技术](/zh/knowledge/training/cadence-shifting-braking)

***

## 报名须知

**请通过本页下方的 RSVP 表单实名报名。** 官网实名报名是为了保障大家的保险权益，也方便我们按人数安排教练与分组。在 Telegram 或微信里说参加不算正式报名。

**参加要求：**

- 一辆状况正常的公路车（或其他适合公路骑行的车型）
- **必须佩戴头盔**
- 能够连续骑行 20 km 以上
- 无需任何团骑经验

安全提示：骑行存在固有风险。课程会系统讲解安全规范并全程有领骑带队，但请每位学员根据自身状况量力而行，对个人安全负责。
