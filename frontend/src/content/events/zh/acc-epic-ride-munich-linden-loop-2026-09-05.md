---
slug: acc-epic-ride-munich-linden-loop-2026-09-05
title: ACC Epic Ride | 慕尼黑 - Linden - Schäftlarn 环线
description: 9 月 6 日周日上午从慕尼黑动物园出发，经 Linden 与 Schäftlarn 完成约 76.4 km 公路环线，限 15 人官网实名报名。
location: 慕尼黑动物园 · Tierparkstraße 30, 81543 München
author: ACC Club
date: "2026-09-06T09:30:00+02:00"
eventType: social-ride
cover: /images/events/acc-epic-ride-munich-linden-loop-2026-09-05/cover.jpg
displaySections:
  - upcoming
status: published
ACCOfficialRide: true
distanceKm: 76.4
maxParticipants: 15
registrationLink: ''
routeKomootUrl: https://www.komoot.com/de-de/tour/3252837959?share_token=a5OfvOBZxmooKxtM0OvC1yD53YDTDvL4bUn06h8zV459RqM2om&ref=wtd&t_s=referral&t_cid=route_share&t_ref_username=1542830902343
---

这次 ACC Epic Ride 从慕尼黑南侧出发，沿公路一路经过 **Linden** 和 **Schäftlarn**，最后回到慕尼黑。路线距离约 **76.4 km**，爬升约 **564 m**，整体属于中等强度的公路骑行。

这不是纯休闲散步线。请大家根据自己的体能、补给准备和独立骑行能力报名。全程以稳定节奏推进，适合有一定公路骑行经验、能完成 70 km 以上路线的骑友。

---

|  |  |
| --- | --- |
| **时间** | **2026 年 9 月 6 日（周日）09:30 集合出发** |
| **集合地点** | **慕尼黑动物园** · Tierparkstraße 30, 81543 München |
| **路线** | 慕尼黑 → Linden → Schäftlarn → 慕尼黑 · [Komoot 查看路线](https://www.komoot.com/de-de/tour/3252837959?share_token=a5OfvOBZxmooKxtM0OvC1yD53YDTDvL4bUn06h8zV459RqM2om&ref=wtd&t_s=referral&t_cid=route_share&t_ref_username=1542830902343) |
| **距离 / 爬升** | 约 **76.4 km / 564 m** |
| **路线类型** | 公路骑行 / 环线 |
| **强度** | 中等强度；需要能稳定完成 70 km 以上公路路线 |
| **人数限制** | **15 人** |
| **报名方式** | 官网活动页实名 RSVP |
| **推荐车辆** | 公路车优先；状态良好的 Gravel Bike 也可 |
| **注意事项** | 头盔必戴；请自带饮水、补给、备胎和基础修车工具 |

---

## 路线预览

- **Komoot 路线**：[查看路线](https://www.komoot.com/de-de/tour/3252837959?share_token=a5OfvOBZxmooKxtM0OvC1yD53YDTDvL4bUn06h8zV459RqM2om&ref=wtd&t_s=referral&t_cid=route_share&t_ref_username=1542830902343)

<iframe src="https://www.komoot.com/de-de/tour/3252837959/embed?profile=1&amp;share_token=a5OfvOBZxmooKxtM0OvC1yD53YDTDvL4bUn06h8zV459RqM2om" width="100%" height="700" frameborder="0" scrolling="no"></iframe>

---

### Ride Notes

- 这是一条从慕尼黑出发并回到慕尼黑的公路环线。
- 路线会经过 **Linden** 与 **Schäftlarn**，整体距离和爬升适合周末中等强度训练。
- 请提前确认车辆状态，尤其是轮胎、刹车和传动系统。
- 请准备足够饮水和补给，不要完全依赖中途临时购买。

---

### 报名 / Join

**请务必通过官网活动页实名报名。**

本次活动人数限制为 **15 人**。只有完成官网实名 RSVP，才视为报名成功。

实名报名是为了保障大家的**保险权益**，请不要只在群里口头报名。

### 保险与费用 / Insurance and Fee

ACC 会为通过官网活动页完成**实名报名**的**非会员**提供活动保险。

非会员参加费用：**非学生 2 欧 / 学生（含博士）1 欧**；**ACC 正式会员免费**。学生请出示学生证，费用于集合时现场支付。
