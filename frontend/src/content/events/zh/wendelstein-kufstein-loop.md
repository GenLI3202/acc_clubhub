---
slug: wendelstein-kufstein-loop
title: ACC 周末骑行 Wendelstein - Kufstein 环线
description: 从 Rosenheim 出发的周末山路环线，约 124.3 km，累计爬升 1538 m。
location: Rosenheim Bahnhof
author: ACC Club
date: 2026-05-30 10:24
registrationDeadline: 2026-05-30T11:00:00+02:00
eventType: social-ride
cover: /images/events/wendelstein-kufstein-loop/cover.jpg
displaySections:
  - upcoming
status: published
ACCOfficialRide: true
---

这次是一条 **周末环线骑行**。
路线从 **Rosenheim** 出发，串联 **Wendelstein** 与 **Kufstein** 一带，整体约 **124.3 km**，累计爬升约 **1538 m**。整体按 **drop ride** 规则执行，请大家根据自己的体能、爬坡能力和补给准备情况决定是否参加。

如果你从慕尼黑出发，建议乘坐 **9:41** 从 **München Hbf** 出发的火车；列车约 **10:24** 抵达 **Rosenheim Bahnhof** 后，大家集合出发。

路线包含一段主要长坡（最高点约 **1100m+**）及后续起伏路段。当天除 **坡顶集合一次** 外，其余路段 **不等人**。建议参加者具备一定爬坡经验，并能独立处理自己的节奏、补给与基础修车问题。

---

|  |  |
| --- | --- |
| **时间** | **5 月 30 日（周六）** |
| **慕尼黑出发建议** | **9:41** 从 **München Hbf** 出发乘火车 |
| **Rosenheim 集合** | **10:24** 火车到达 **Rosenheim Bahnhof** 后集合出发 |
| **形式** | **Rosenheim 出发环线骑行** |
| **路线规模** | 约 **124.3 km** · 累计爬升约 **1538 m** · [Komoot 查看路线](https://www.komoot.com/tour/2996278072?ref=aso&share_token=a6htnvhcLhQsz3LDgWrH8XV1UaVyMDurHAkJWNxoMykpl6PRpp&t_s=referral&t_cid=route_share) |
| **骑行规则** | **Drop ride / 不等人** |
| **难度提醒** | 包含一段主要长坡；仅在 **坡顶集合一次**；建议有一定爬坡经验 |
| **报名规则** | 按此前官网活动页邮件报名方式处理 |

---

### 路线预览

<iframe src="https://www.komoot.com/tour/2996278072/embed?share_token=a6htnvhcLhQsz3LDgWrH8XV1UaVyMDurHAkJWNxoMykpl6PRpp&profile=1" width="100%" height="700" frameborder="0" scrolling="no"></iframe>

---

### 参加前提醒

- 本次按 **drop ride** 规则进行
- 请提前准备好补给、外套、修车工具和应急方案
- 如长坡经验还不太稳定，建议先谨慎评估再报名

### 报名 / Join

**请务必通过官网活动页实名报名。**
实名报名是为了保障大家的 **保险权益**，这一点不要省略。

### 保险与费用 / Insurance and Fee

ACC 会为通过官网活动页完成 **实名报名** 的 **非会员** 提供活动保险。

非会员参加费用：**非学生 2 欧 / 学生（含博士）1 欧**；**ACC 正式会员免费**。学生请出示学生证，费用于集合时现场支付。
