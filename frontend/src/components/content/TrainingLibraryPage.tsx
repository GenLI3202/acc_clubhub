// components/content/TrainingLibraryPage.tsx
// Interactive "ALL ARTICLES" section for the training library page.
// Reuses FilterPanel + filter hooks unchanged; renders ArticleCard grid instead of MasonryGrid.
...
interface TrainingLibraryPageProps {
  initialItems: any[];
  lang: Locale;
  initialFilters?: Record<string, any>;
}

export default function TrainingLibraryPage({ initialItems, lang, initialFilters = {} }: TrainingLibraryPageProps) {
  ...
  const emptyMsg = lang === 'zh' ? '没有找到匹配的内容' : lang === 'de' ? 'Keine passenden Inhalte' : 'No matching content';
  const clearLabel = lang === 'zh' ? '清除筛选' : lang === 'de' ? 'Filter zurücksetzen' : 'Clear Filters';
        ...
        <div class="article-grid">
          {filteredItems.map((entry) => {
            ...
            return (
              <ArticleCard
                key={data.slug}
                href={`/${lang}/knowledge/training/${data.slug}`}
                title={data.title}
